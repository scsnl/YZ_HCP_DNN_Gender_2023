...
def data_cleaning_leipzig(path_to_dataset):
    # load & clean data
    datao = pd.read_pickle(path_to_dataset)
    datao.drop(datao[datao['percentofvolsrepaired'] > 10].index, inplace=True)
    datao.drop(datao[datao['mean_fd'] > 0.5].index, inplace=True)
...
def get_features_labels(path_to_dataset, site):
    if site == 'hcp':
        ...
        data, labels = data_cleaning_leipzig(path_to_dataset)
...
if __name__ == '__main__':
    ...
    hcpData = 'PUBLIC_DATA_DIR/hcp/restfmri/timeseries/group_level/brainnetome/' \
                      'normz/hcp_run-rfMRI_REST1_LR_brainnetome_mean_regMov-6param_wmcsf_dt1_bpf008-09_normz_246ROIs.pklz'
    ...
    testData1 = 'PUBLIC_DATA_DIR/mpi_leipzig/restfmri/timeseries/group_level/brainnetome/normz/mpi_leipzig_run-AP_run-01_brainnetome_mean_regMov-6param_wmcsf_dt1_bpf008-09_normz_246ROIs.pklz'
    print("models trained on HCP Session 1, tested on leipzig_AP_run01")
    model_ss = 'HCP_Session1'
    ...
    f2 = output_path + 'sFC_leipzig_AP_run01.npz'
    ...
    testData1_features, testData1_labels = get_features_labels(testData1, 'leipzig')
    np.savez(f2, features=testData1_features, labels=testData1_labels)
    ...
    for name, model in models:
        ...
        # performance on test1 (leipzig) dataset
        print('** Mean leipzig accuracy across 5 folds {0:.2f} +/- {1:.2f} %'.format(np.mean(test1_accuracy),np.std(test1_accuracy)))
        print('** Mean leipzig precision across 5 folds {0:.2f} +/- {1:.2f} %'.format(np.mean(test1_precision),np.std(test1_precision)))
        print('** Mean leipzig recall across 5 folds {0:.2f} +/- {1:.2f} %'.format(np.mean(test1_recall), np.std(test1_recall)))
        print('** Mean leipzig f1-score across 5 folds {0:.2f} +/- {1:.2f} %'.format(np.mean(test1_f1score),np.std(test1_f1score)))
