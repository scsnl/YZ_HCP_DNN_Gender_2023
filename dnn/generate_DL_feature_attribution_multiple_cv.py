...
if __name__ == "__main__":
    ...
    # output
    output_path = 'PROJECT_DIR/results/restfmri/dnn/attributions_deeplift/multiple_cv/'
        ...
        for jj in range(num_5folds):
            ...
            for m in range(5): # 5 splits/folds (5 models); get features attributions for each model
                ...
                for i in range(len(input_tensor_test)):
                    ...
                    attr = getInputAttributions_DeepLift(model, input_tensor_test[i].unsqueeze_(-1).permute(2,0,1), label_tensor_test[i]) # female = 1; male = 0
                    features.append(attr)

                features = np.concatenate(features)

                feature_attribution_fname = output_path + 'ff' + str(jj) + '_hcp_model_' + model_session + '_index_' + str(m) + '_test_' + test_session + '.npz'
