...
if __name__ == '__main__':
    ...
    for ii in range(2):
        ...
        output_file = output_path + 'hcp_model_' + model_session + '_test_leipzig_' + test_session + '_consensus_multiple_cv.xlsx'
        ...
        for percentile in [80]:#[80, 85, 90, 95]: # for each percentile
            for group in groups.keys(): # for each gender
                ...
                site = 'hcp_model_' + model_session + '_test_leipzig_' + test_session

                for jj in range(num_5folds):
                    print("the {} 5folds".format(jj))

                    for m in range(5):
                        data_file = data_path + 'ff' + str(jj) + '_hcp_model_' + model_session + '_index_' + str(m) + '_test_leipzig_' + test_session + '.npz'
